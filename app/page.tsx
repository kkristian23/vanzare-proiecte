...
const filters = ["Toate", "Beauty", "SaaS", "E-commerce", "Landing", "Dashboard"];
...
export default function Home() {
  ...
  return <main>
    ...
    <AnimatePresence>{selected && <motion.div className="modal-wrap" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={()=>setSelected(null)}><motion.div className="modal" initial={{y:30,scale:.97}} animate={{y:0,scale:1}} exit={{y:20,opacity:0}} onClick={e=>e.stopPropagation()}><button className="modal-close" onClick={()=>setSelected(null)}><X/></button><ProjectVisual project={selected}/><span className="kicker">{selected.type} / LICENȚĂ COMPLETĂ</span><h2>{selected.title}</h2><p>{selected.id === 7 ? "Site premium pentru salon de frumusețe, cu identitate editorială delicată, experiență trilingvă, prezentare interactivă a serviciilor, galerie și formular de programare. Conceput pentru a transforma vizitatorii în programări." : `${selected.desc}. Primești codul sursă complet, documentație și ajutor la lansare.`}</p><ul><li><Check/> Cod sursă complet și editabil</li><li><Check/> {selected.id === 7 ? "Română, rusă și engleză incluse" : "Personalizare de bază inclusă"}</li><li><Check/> {selected.id === 7 ? "Servicii, galerie și programări" : "30 de zile de suport"}</li></ul><a href={`mailto:hello@monodev.ro?subject=Interesat de ${selected.title}`}>Cumpără pentru €{selected.price} <ExternalLink/></a></motion.div></motion.div>}</AnimatePresence>
  </main>;
}
