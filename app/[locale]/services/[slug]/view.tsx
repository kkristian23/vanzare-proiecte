...
import { absoluteUrl, type Locale, localePath } from "../../../lib/site-config";
...
export default function ServiceView({ locale, slug }: { locale: Locale; slug: string }) {
  ...
  return <SeoShell locale={locale} path={`services/${slug}`} title={copy.title} description={copy.description}>
    ...
    <section className="seo-section"><h2>{labels.examples}</h2><p>{labels.examplesNote}</p><div className="seo-grid">{examples.map((project) => <article className="seo-card" key={project.id}>
      <h3><a href={projectHref(locale, project.id)}>{project.title}</a></h3><p>{project.description}</p><a href={projectHref(locale, project.id)}>{labels.detail} {cmsText("view-services-slug", "literal-1273c66ca1ec0211", " →")}</a>
    </article>)}</div></section>
    ...
    <section className="seo-section seo-cta"><h2>{labels.cta}</h2><a href={localePath(locale, "contact")}>{labels.cta} {cmsText("view-services-slug", "literal-1273c66ca1ec0211", " →")}</a><p><a href={localePath(locale, "services")}>{labels.all}</a></p></section>
