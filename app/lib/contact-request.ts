...
export function contactOptionLabel(locale: Locale, option: string): string {
  const copy = cmsContent("contact-request-options", {
    ro: { purchase: "achiziție", rental: "chirie", installments: "rate", months: "luni", unspecified: "de discutat" },
    ru: { purchase: "покупка", rental: "аренда", installments: "рассрочка", months: "месяцев", unspecified: "обсудим условия" },
    en: { purchase: "purchase", rental: "rental", installments: "instalments", months: "months", unspecified: "to be discussed" },
  })[locale];
  if (option === "purchase") return copy.purchase;
  if (option === "rental" || option === "rent") return copy.rental;
  if (option === "site-rental") return copy.rental;
